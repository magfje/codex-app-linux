...
function patchDynamicToolThreadStartBridge(source, patch = findDynamicToolThreadStartBridgePatch(source)) {
  ...
  const helper = source.includes(dynamicToolThreadStartBridgeMarker)
    ? ""
    : [
        `function ${dynamicToolThreadStartBridgeMarker}(e){if(e?.method!==\`thread/start\`||e.params==null)return e;let t=__codexLinuxNormalizeThreadStartBridgeValue(e.params);return{...e,params:{...t,...Array.isArray(t.dynamicTools)?{dynamicTools:t.dynamicTools.map(__codexLinuxNormalizeThreadStartBridgeDynamicTool)}:{}}}}`,
        ...
        `function __codexLinuxNormalizeThreadStartBridgeDynamicTool(e){e=__codexLinuxNormalizeThreadStartBridgeValue(e);if(e==null||typeof e!==\`object\`)return e;if(Array.isArray(e.tools))return{...e,tools:e.tools.map(__codexLinuxNormalizeThreadStartBridgeDynamicTool)};return e.type==null&&typeof e.name===\`string\`?{...e,type:\`function\`,inputSchema:e.inputSchema??{type:\`object\`,properties:{},additionalProperties:!1}}:e}`
      ].join("");
  const replacements = patch.patches.map(target => {
